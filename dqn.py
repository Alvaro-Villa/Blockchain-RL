...
class DQN(object):
    ...
    def __init__(self, max_memory = 100, discount_factor = 0.9): # Discount factor ya esta testeado
        self.memory = list()
    ...
    def get_batch(self, model, batch_size = 10):
        ...
        num_inputs = self.memory[0][0][0].shape[1] # 1º transicion 2º gameover 3º transferencia de datos
        num_outputs = model.output_shape[-1]
        inputs = np.zeros((min(batch_size, len_memory), num_inputs))  # utilizamos min para adaptar la dim de la matriz      
        targets = np.zeros((min(batch_size, len_memory), num_outputs))
        for i, idx in enumerate(np.random.randint(0, len_memory, size=min(len_memory, batch_size))):
            current_state, action, reward, next_state = self.memory[idx][0] # enumerate
